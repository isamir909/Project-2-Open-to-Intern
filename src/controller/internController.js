...
const createIntern = async function (req, res) {
    try {
        ...
        if (!("name" in data) )return res.status(400).send({ status: false, msg: "Don't Skip The Name Attribute" })
        if (!("email" in data) )return res.status(400).send({ status: false, msg: "Don't Skip The email Attribute" })
        if (!("mobile" in data) )return res.status(400).send({ status: false, msg: "Don't Skip The mobile Attribute" })
        if (!("collegeName" in data) )return res.status(400).send({ status: false, msg: "Don't Skip The collegeName Attribute" })
